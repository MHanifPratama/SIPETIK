...
function update_rekap($data){
   ...
   $data_bus = mysqli_query($conn,"SELECT * FROM tiket,bus, tipe_bus,perjalanan,jadwal where tiket.id_bus = bus.id_bus AND bus.id_tipe = tipe_bus.id_tipe AND bus.id_perjalanan = perjalanan.id_perjalanan AND bus.id_jadwal = jadwal.id_jadwal AND tiket.id_tiket = $id");
   $data_aa = mysqli_query($conn,"SELECT COUNT(bus.id_bus) AS num FROM tiket,bus, tipe_bus,perjalanan,jadwal where tiket.id_bus = bus.id_bus AND bus.id_tipe = tipe_bus.id_tipe AND bus.id_perjalanan = perjalanan.id_perjalanan AND bus.id_jadwal = jadwal.id_jadwal AND tiket.id_tiket = $id");
   ...
   endforeach;  
   // mysqli_query($conn,"INSERT INTO laporan VALUES ('','$tanggal','$numUsers','$a')");
   $quer = "UPDATE laporan SET total_tiket = '$numUsers',total_harga = '$a' WHERE id_laporan = $id";
   return query($quer);
}
?>